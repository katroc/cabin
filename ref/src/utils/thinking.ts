...
export function deriveAnswerFromThinking(thinking: string): string {
  ...
  // Heuristic 4: Extract factual content from thinking that mentions the topic
  // Look for sentences that seem to contain factual answers
  const lines = t.split('\n').map(line => line.trim()).filter(line => line.length > 0);
  const factualLines: string[] = [];
  
  for (const line of lines) {
    // Skip meta-commentary about the process
    if (/^(we need|we should|we must|according to|let's|looking at|the question|the user|based on|from context)/i.test(line)) {
      continue;
    }
    
    // Look for lines that contain factual statements with citations
    if (/\[\d+\]/.test(line) && line.length > 30) {
      factualLines.push(line);
    }
    
    // Look for lines that define or describe the subject
    if (/^[A-Z][a-z]+ is|^[A-Z][a-z]+ provides|^[A-Z][a-z]+ offers|^It is|^It provides/.test(line)) {
      factualLines.push(line);
    }
  }
  
  // If we found substantial factual content, format it as an answer
  if (factualLines.length >= 2) {
    return factualLines.join(' ').replace(/\s+/g, ' ').trim();
  }

  // Heuristic 5: If thinking is substantial but no clear answer patterns,
